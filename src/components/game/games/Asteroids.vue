...
import Agent from '../../../games/asteroids/agent.js';
import Asteroid from '../../../games/asteroids/asteroid.js';
      ...
      pFive.draw = () => {
        ...
        for (let n = 0; n < speed; n++) {
          ...
          for (let i = activeAgents.length - 1; i >= 0; i--) {
            ...
            agent.think(asteroids);
            agent.update();
            // Check all the asteroids
            for (let j = 0; j < asteroids.length; j++) {
              // It's hit a pipe
              if (asteroids[j].hits(activeAgents[i])) {
                // Remove this agent
                // TODO: Die reward comes elsewhere
                activeAgents[i].afterAction(asteroids, -1000);
